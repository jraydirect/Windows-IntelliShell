...
class AIBridge:
    ...
    def _build_system_prompt(self) -> str:
        ...
        prompt_parts.extend([
            ...
            '{"intent": "intent_name", "provider": "provider_name", "parameters": {"key": "value"}, "confidence": 0.0-1.0, "reasoning": "brief"}',
            "",
            "Parameter Extraction:",
            "- For launch_app: extract app name as 'app_name' in parameters (e.g., 'open brave' → parameters: {'app_name': 'brave'})",
            "- For other intents: extract relevant values into parameters object",
            "",
            "Rules:",
            ...
            "- For 'clipboard' queries (show/list/search clipboard) → use clipboard provider intents (show_clipboard_history, search_clipboard)",
            ...
            "- For 'top N processes' / 'most resource draining processes' / 'processes using most memory' → ALWAYS use list_processes (READ-ONLY), NEVER kill_most_memory",
            "- For 'what are the top X processes' → use list_processes with count parameter (READ-ONLY)",
            "- kill_most_memory is ONLY for actually killing/terminating a process, NOT for listing/viewing",
            ...
            "- Questions asking 'what' / 'show' / 'list' / 'top' / 'most' → use READ-ONLY list_* intents, NOT kill/delete intents",
            "- NEVER route viewing/listing queries to destructive actions (kill, delete, etc.)",
            "- When user asks 'what are the top X processes' or 'most resource draining' → they want to LIST/VIEW, not KILL",
            "- kill_most_memory is a DESTRUCTIVE action that terminates processes - only use when user explicitly wants to KILL/TERMINATE",
            ...
            "- IMPORTANT: 'clipboard history' queries should use clipboard provider, NOT memory provider!",
            "",
            "Examples:",
            ...
            '"what are the top 5 most resource draining processes?" → {"intent": "list_processes", "provider": "system", "confidence": 0.95, "reasoning": "list top processes"}',
            '"top 10 processes by memory" → {"intent": "list_processes", "provider": "system", "confidence": 0.95, "reasoning": "list processes"}',
            '"show me processes using most memory" → {"intent": "list_processes", "provider": "system", "confidence": 0.95, "reasoning": "list processes"}',
            '"show me my clipboard history" → {"intent": "show_clipboard_history", "provider": "clipboard", "confidence": 0.95, "reasoning": "show clipboard"}',
            '"my most recent clipboard item" → {"intent": "show_clipboard_history", "provider": "clipboard", "confidence": 0.95, "reasoning": "show recent clipboard"}',
            '"search clipboard for password" → {"intent": "search_clipboard", "provider": "clipboard", "confidence": 0.95, "reasoning": "search clipboard"}',
            '"open brave" → {"intent": "launch_app", "provider": "app", "parameters": {"app_name": "brave"}, "confidence": 0.95, "reasoning": "launch application"}',
